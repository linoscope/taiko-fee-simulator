...
(function () {
  ...
  if (!l1X.length || !l2ArbX.length || !l2BaseX.length) {
    setStatus('Dataset is empty.');
    return;
  }

  const MIN_X = Math.max(l1X[0], l2ArbX[0], l2BaseX[0]);
  const MAX_X = Math.min(l1X[l1X.length - 1], l2ArbX[l2ArbX.length - 1], l2BaseX[l2BaseX.length - 1]);
  ...
  function makeOpts(title, seriesLabel, strokeColor, width, height) {
    return {
      title: title,
      width: width,
      height: height,
      ...
      series: [
        {},
        { label: seriesLabel, stroke: strokeColor, width: 1.2 }
      ],
      axes: [
  ...
  function allPlots() {
    return [l1BasePlot, l1BlobPlot, l2ArbBasePlot, l2BaseBasePlot].filter(Boolean);
  }
  ...
  l1BasePlot = new uPlot(
    makeOpts('L1 Base Fee History', 'L1 base fee (gwei)', '#2563eb', plotWidthFor(l1BaseWrap), 300),
    [l1X, l1Base],
    l1BaseWrap
  );

  l1BlobPlot = new uPlot(
    makeOpts('L1 Blob Fee History', 'L1 blob fee (gwei)', '#f97316', plotWidthFor(l1BlobWrap), 300),
    [l1X, l1Blob],
    l1BlobWrap
  );

  l2ArbBasePlot = new uPlot(
    makeOpts('L2 Base Fee History (Arbitrum)', 'L2 base fee (gwei)', '#0f766e', plotWidthFor(l2ArbBaseWrap), 300),
    [l2ArbX, l2ArbBase],
    l2ArbBaseWrap
  );

  l2BaseBasePlot = new uPlot(
    makeOpts('L2 Base Fee History (Base)', 'L2 base fee (gwei)', '#dc2626', plotWidthFor(l2BaseBaseWrap), 300),
    [l2BaseX, l2BaseFee],
    l2BaseBaseWrap
  );
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate a synced uPlot HTML for L1 base/blob + L2 (Arbitrum/Base) base fee histories on timestamp axis."
    )
    ...
    parser.add_argument(
        "--out-html",
        default=str(plots_dir / "fee_history_l1_l2_synced.html"),
        help="Output HTML path",
    )
    parser.add_argument(
        "--out-data-js",
        default=str(plots_dir / "fee_history_l1_l2_synced_data.js"),
        help="Output data JS path",
    )
    parser.add_argument(
        "--out-summary",
        default=str(plots_dir / "fee_history_l1_l2_synced_summary.json"),
    ...
    if args.l1_csv:
        ...
        l1_csv_path = find_latest_nontrivial_csv(data_dir, "eth_l1_fee_365d_*.csv")
    ...
    if args.l2_csv:
        ...
        l2_arb_csv_path = find_latest_nontrivial_csv(data_dir, "arb1_fee_*_step*_*.csv")
    ...
    if args.l2_base_csv:
        ...
        l2_base_csv_path = find_latest_nontrivial_csv(data_dir, "base_fee_*_step*_*.csv")
    ...
    window_min = max(l1["x_sec"][0], l2_arb["x_sec"][0], l2_base["x_sec"][0])
    window_max = min(l1["x_sec"][-1], l2_arb["x_sec"][-1], l2_base["x_sec"][-1])
    ...
    payload = {
        "meta": {
            ...
            "l1": {
                ...
                "summary_window_start_utc": l1_summary.get("assumed_start_timestamp_utc")
                or l1_summary.get("window_start_timestamp_utc"),
