...
def main():
    parser = argparse.ArgumentParser(
        description="Fetch Taiko historical base fee/gas usage over a time window with throttled batched RPC."
    )
    parser.add_argument(
        "--rpc",
        default="https://rpc.mainnet.taiko.xyz",
        help="Taiko JSON-RPC endpoint",
    )
    parser.add_argument(
        "--window-summary",
    ...
    print("Locating Taiko block bounds for timestamp window...")
    ...
    csv_base = f"taiko_fee_{start_block}_{end_block}_step{stride_blocks}_{stamp}"
