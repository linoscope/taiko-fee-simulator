...
def rpc_batch_get_blocks(
    ...
    def rpc_single_get_block(block_number: int):
        payload = {
            "jsonrpc": "2.0",
            "id": block_number,
        ...
        raise RuntimeError(
            f"Failed fetching block {block_number} from {rpc_url}"
        ) from last_exc
    ...
    for i in range(0, len(block_numbers), request_batch_size):
        ...
        for retry_i in range(retries):
            try:
                ...
                if isinstance(arr, dict):
                    # Some providers return a non-batch error payload even with HTTP 200
                    # when batch requests are not supported.
                    fallback_to_single = True
                    break
                if not isinstance(arr, list):
                    raise RuntimeError(f"Unexpected batch response shape: {type(arr)}")
                by_id = {}
                for item in arr:
        ...
        if fallback_to_single:
            ...
            for bn in chunk:
                ...
                if wait > 0:
                    time.sleep(wait)
                out.append(rpc_single_get_block(bn))
                last_request_at = time.monotonic()
            continue
...
def main():
    parser = argparse.ArgumentParser(
        description="Fetch Optimism historical base fee/gas usage over a time window with throttled batched RPC."
    )
    parser.add_argument("--rpc", default="https://mainnet.optimism.io", help="Optimism JSON-RPC endpoint")
    parser.add_argument(
        "--window-summary",
    ...
    print("Locating Optimism block bounds for timestamp window...")
    ...
    csv_base = f"optimism_fee_{start_block}_{end_block}_step{stride_blocks}_{stamp}"
